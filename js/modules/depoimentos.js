export default class CriarDepoimentos {
  constructor(listaDepoimentos) {
    this.depoimentosContainer = document.querySelector(listaDepoimentos);
    this.url = "./depoimentos-dos-usuarios/depoimentos.json";
  };
  async fetchDepoimentos() {
    try {
      ...
      responseJSON.forEach((item) => {
        ...
        li.appendChild(p);
        li.appendChild(h3);
        this.depoimentosContainer.appendChild(li);
      });
    } catch (error) {
      console.log(error);
    };
